...
class Board:
    def __init__(self, players: List[Player]):
        ...
        self.p_to_l = {players[0]: {(0, 0): Rook(players[0], 0), (0, 7): Rook(players[0], 1),
                       ...
                       players[1]: {(7, 0): Rook(players[0], 0), (7, 7): Rook(players[0], 1),
                                    (7, 1): Knight(players[0], 0), (7, 6): Knight(players[0], 0),
                                    (7, 2): Bishop(players[0], 0), (7, 5): Bishop(players[0], 1),
                                    (7, 3): Queen(players[0], 0), (7, 4): Knight(players[0], 0),
                                    (6, 0): Pawn(players[0], 0), (6, 1): Pawn(players[0], 1),
                                    (6, 2): Pawn(players[0], 2), (6, 3): Pawn(players[0], 3),
                                    (6, 4): Pawn(players[0], 4), (6, 5): Pawn(players[0], 5),
                                    (6, 6): Pawn(players[0], 6), (6, 7): Pawn(players[0], 7)}}
        self.l_to_p = {players[0]: {Rook(players[0], 0): (0, 0), Rook(players[0], 1): (0, 7),
                       ...
                       players[1]: {Rook(players[0], 0): (7, 0), Rook(players[0], 1): (7, 7),
                                    Knight(players[0], 0): (7, 1), Knight(players[0], 0): (7, 6),
                                    Bishop(players[0], 0): (7, 2), Bishop(players[0], 1): (7, 5),
                                    Queen(players[0], 0): (7, 3), Knight(players[0], 0): (7, 4),
                                    Pawn(players[0], 0): (6, 0), Pawn(players[0], 1): (6, 1),
                                    Pawn(players[0], 2): (6, 2), Pawn(players[0], 3): (6, 3),
                                    Pawn(players[0], 4): (6, 4), Pawn(players[0], 5): (6, 5),
                                    Pawn(players[0], 6): (6, 6), Pawn(players[0], 7): (6, 7)}}
    ...
    def restricted_check(self, player, other_player):
        ...
        b1 = self.p_to_l.get(other_player).get(Rook(other_player, 0), None)
        b2 = self.p_to_l.get(other_player).get(Rook(other_player, 1), None)
        r1 = self.p_to_l.get(other_player).get(Rook(other_player, 0), None)
        r2 = self.p_to_l.get(other_player).get(Rook(other_player, 1), None)
        q = self.p_to_l.get(other_player).get(Queen(other_player, 1), None)
        for node in [k1, k2, b1, b2]:
            if node is not None:
        ...
        for node in (r1, r2):
            if node is not None:
                if node[0] == king_loc[0] or node[1] == king_loc[1]:
                    pieces.append(node)
        if ((q[0] + q[1]) % 2) == ((king_loc[0] + king_loc[1]) % 2) or q[0] == king_loc[0] or q[1] == king_loc[1]:
            pieces.append(q)
        i, j = king_loc
    ...
    def pre_processing(self, player: Player):
        ...
        for piece in p_to_l.values():
            piece = p_to_l.get(piece)
            nodes = self.board[piece[0]][piece[1]].top.check_move(self.board, piece)
            if nodes is not None:
                nodes = self.achmaz_detection(player, piece, nodes, restricted)
                stick.extend(nodes)
                result[piece] = nodes
        if len(stick) == 0:
